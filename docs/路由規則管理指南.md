...
4. **建立任務**：對於每個命中的 `actions`，調度器組合任務 payload（分析方法、參數 config、目標 MongoDB 實例等）並以 `analysis.<analysis_method_id>` 的 routing key 發送到 RabbitMQ（`core/state_management/utils/rabbitmq_handler.py`）。  
